...
func (auth *authHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if _, err := r.Cookie("gammaAuth"); err == http.ErrNoCookie { // not authenticated
...
//handling the third party login process
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "login":
		...
		if err != nil {
			log.Fatalln("Error when trying to get BeginAuth", provider, "-", err.Error())
		}
